"""Build a styled, multi-sheet .xlsx report from a structured JSON input.

This generalizes scripts/create_spreadsheet.py into a data-driven builder
(mirroring scripts/build_report.py for docx): instead of one hardcoded
sample table, the workbook's sheets (each with its own headers/rows) are
supplied via a JSON file, while the visual styling and the ID-column /
quote-italic / auto-chart behavior stay consistent and automatic.

Usage:
    python build_spreadsheet.py --input findings.json --output report.xlsx [--color "#2E86AB"]
...
JSON input schema:
{
  ...
  "sheets": [
    {"name": "Overview", "headers": ["Item", "Note"], "rows": [["A", "..."]]},
    {"name": "Data", "headers": ["Year", "Growth Rate"], "rows": [["2023", "12.5%"]]},
    {"name": "Sources", "headers": ["Source", "URL"], "rows": [["...", "https://..."]]}
  ]
}

Each sheet is independent: its own header row, its own ID column (skipped
if the sheet's first header is already an identifier like ID/No/번호), its
own quote-italic columns (header named quote/quotation/인용구/인용문), and
its own auto-generated chart if it has a real numeric comparison/growth
column. The title and subtitle banner is only drawn on the first sheet.

Styling defaults (mirrors scripts/create_spreadsheet.py):
    - Font: 맑은 고딕 (Malgun Gothic)
    - Title: 24pt bold, main color (first sheet only)
...
def add_comparison_charts(wb, ws, headers, data_rows, header_row, last_col):
    ...
    chart_ws = wb.create_sheet(_unique_sheet_name(wb, "Charts")) if use_separate_sheet else ws
...
def _unique_sheet_name(wb, desired_name: str) -> str:
    """Sanitize a sheet name for Excel and make sure it's unique in wb."""
    base = INVALID_SHEET_CHARS.sub(" ", str(desired_name)).strip() or "Sheet"
    base = base[:MAX_SHEET_NAME_LENGTH]
    name = base
    suffix = 2
    existing = set(wb.sheetnames)
    while name in existing:
        trimmed = base[: MAX_SHEET_NAME_LENGTH - len(f" ({suffix})")]
        name = f"{trimmed} ({suffix})"
        suffix += 1
    return name


def populate_sheet(wb, ws, sheet_data: dict, main_color: str, show_banner: bool, title_text: str = "", subtitle_text: str = ""):
    """Fill one worksheet with a styled table (+ ID column, quote italics,
    body 10pt, auto chart) built from {"headers": [...], "rows": [[...]]}.
    """
    raw_headers = sheet_data.get("headers", [])
    raw_rows = sheet_data.get("rows", [])
    headers, rows = add_id_column(raw_headers, raw_rows)
    ...
    add_comparison_charts(wb, ws, headers, rows, header_row, last_col)


def build_workbook(data: dict, color_override: str = None) -> Workbook:
    title_text = data.get("title", "Spreadsheet Title")
    subtitle_text = data.get("subtitle", "")
    main_color = color_override or parse_color(data.get("color", DEFAULT_COLOR))
    sheets = data.get("sheets") or [{"name": "Report", "headers": [], "rows": []}]

    wb = Workbook()
    wb.remove(wb.active)

    for sheet_index, sheet_data in enumerate(sheets):
        sheet_name = _unique_sheet_name(wb, sheet_data.get("name") or f"Sheet{sheet_index + 1}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build a styled, multi-sheet .xlsx report from a JSON input.")
    parser.add_argument("--input", type=Path, required=True, help="Path to the input JSON file")
    parser.add_argument("--output", type=Path, default=Path("report.xlsx"), help="Output .xlsx path")
    parser.add_argument(
        "--color",
        type=parse_color,
        default=None,
...
def main() -> None:
    ...
    workbook = build_workbook(data, color_override=args.color)
